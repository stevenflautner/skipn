...
actual interface Builder {
    ...
    fun createContextAndDescend(id: String, routeLevel: Int = getBuildContext().getRouteLevel()): BuildContext {
        // Creates a copy of the states
        val context = BuildContext.create(id, getBuildContext(), routeLevel)
    ...
    fun descendBuilder(context: BuildContext) {
        context.scope.attach(getBuildContext().scope)
    ...
    private fun ascendContext(elemId: String) {
        getBuildContext().pinningContext.ascend(elemId)
...
//            if (builderContextTree.size == 1) return
            ...
            currentBuildContext = builderContextTree.lastOrNull()
        }
    }
